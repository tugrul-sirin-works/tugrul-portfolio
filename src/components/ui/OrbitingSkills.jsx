...
// İkonlar (Senin verdiğin koddan alındı)
...
// Burayı senin yeteneklerine göre güncelleyebiliriz (Amazon, Entegra vb.)
const skillsConfig = [
...
const OrbitingSkill = memo(({ config, angle }) => {
    ...
    return (
        <div className="absolute top-1/2 left-1/2 transition-all duration-300 ease-out"
            ...
            <div className={`relative w-full h-full p-2 bg-gray-800/90 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer ${isHovered ? 'scale-125 shadow-2xl' : 'shadow-lg hover:shadow-xl'}`}
                ...
                {isHovered && <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900/95 backdrop-blur-sm rounded text-xs text-white whitespace-nowrap pointer-events-none">{label}</div>}
            </div>
        </div>
    );
});
...
export default function OrbitingSkills() {
    ...
    const orbitConfigs = [{ radius: 100, glowColor: 'cyan', delay: 0 }, { radius: 180, glowColor: 'purple', delay: 1.5 }];
    ...
    return (
        <div className="relative w-full h-[600px] flex items-center justify-center overflow-hidden">
            <div className="relative w-full h-full flex items-center justify-center" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
                {/* Merkez Logo */}
                <div className="w-24 h-24 bg-gradient-to-br from-brand-accent to-purple-600 rounded-full flex items-center justify-center z-10 relative shadow-2xl border-4 border-black/50">
